# -*- coding: utf-8 -*-
"""
Validacion del modelo PowerFactory — MAS X MENOS COLOMBIA (circuito 58-512).
...
Salida: ../results/validacion_pf.txt
"""
...
PROJECT_NAME = r"\Usuario Principal.IntUser\Mas X Menos Colombia.IntPrj"
STUDY_CASE = None  # None = caso activo

SITE = "COLOMBIA"
CIRCUITO = "58-512"
V_NOM_KV = 13.8
POC_NAME = ""  # completar con el nodo POC cuando se defina

# Conteos esperados segun Excel OR (Datos Circuito 58 512.xlsx)
EXPECTED_LINES = 165
EXPECTED_LOADS = 62
TOL_COUNT = 0.15
...
RUN_LOAD_FLOW = True
RUN_SHORT_CIRCUIT = False  # activar cuando POC_NAME este definido

_SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = _SCRIPT_DIR.parent / "results"
...
def run_load_flow(app, report: list[str]) -> list[str]:
    ...
    for ln in app.GetCalcRelevantObjects("*.ElmLne") or []:
        loading = safe_attr(ln, "c:loading") or safe_attr(ln, "m:loading")
        if loading is None:
            continue
        try:
...
def run_short_circuit(app, report: list[str]) -> list[str]:
    issues = []
    report.append("")
    report.append(f"--- Cortocircuito IEC 60909 (POC={POC_NAME or 'N/D'}) ---")
    if not POC_NAME:
        report.append("AVISO: POC_NAME vacio — se omite CC")
        return issues
    ...
    if ik is None:
        report.append("AVISO: no se leyeron variables m:Ik / m:Ikss")
    else:
...
def main():
    ...
    report = [
        ...
        f"Vnom: {V_NOM_KV} kV | POC: {POC_NAME or '(definir)'}",
        "=" * 70,
    ]
